...
public class dashControlleruploadadmin {
    ...
    public void accept(ActionEvent actionevent) throws IOException {
        pesan.setPesan(coment.getText());
        Parent root = FXMLLoader.load(getClass().getResource("FXMLpesanadmin.fxml"));
    ...
    public void reject(ActionEvent actionevent) throws IOException, SQLException {
        Connection conn = databaseuploadmahasiswa.getConnection();
        conn.createStatement().executeUpdate("DELETE FROM user WHERE Nim1='" + carijudul.getText() + "' "); 
        Parent root = FXMLLoader.load(getClass().getResource("FXMLpesanadmin.fxml"));
        stage = (Stage) ((Node) actionevent.getSource()).getScene().getWindow();
        scene = new Scene(root);
        stage.setScene(scene);
